...
void main( void )
{
    ...
    vec4 tOak = t_(TexOAK, passUVs.xy);
    float humanInFrame = 0.;
    vec4 O=t_(Texfeedback, fract(U/(u_resImg.xy - oSpeed)))+sin(U.xyyy/50.)/(u_time);
    if(length(tOak.rgb) >0.)
    {
        O.rgb = t_(TexOAK, fract(U/(u_resImg.xy - oSpeed))).rgb+sin(U.xyy/50.)/(u_time);
        humanInFrame = 1.0;
    }
    float amnt = 7. * abs(sin(u_time) ) + 6.;
    ...
    for(float i = 0.;i< amnt ;i+=1.0){
        ...
        if(humanInFrame == 1.){
            O -= (1. + .1 * sin(u_time * .1) ) * t_(TexOAK, v2 );
            O += (1. + .1 * cos(u_time * .3) ) * t_(TexOAK, v1 );
        }
	...
	if(humanInFrame == 0.){
        ...
        float mixProp = prop * .05;
        O = normalize(t_(Texfeedback, fract((U + O.yx)/u_resImg.xy)) * (1. - mixProp) + mixProp * t_(TexOAK, fract((U + 2. * O.yx)/u_resImg.xy)).xyzx +.02 * cos(u_time *.4)*O.wxyz);
        O.rgb =O.rgb* .9+.1* miniTransformColors(O.rgb, u_time * 5., passUVs.y).rgb;
    }
